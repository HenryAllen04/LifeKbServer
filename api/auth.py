...
class handler(BaseHTTPRequestHandler):
    ...
    def _handle_login(self, data: Dict):
        ...
        try:
            ...
            error_message = str(e)
            print(f"Login error for {email}: {error_message}")  # Debug logging
            ...
            elif "Email not confirmed" in error_message or "signup_disabled" in error_message:
    ...
    def _handle_register(self, data: Dict):
        ...
        try:
            ...
            # Extract user info from Supabase response
            # Check if user is in response or top level
            user_data = response.get("user", {})
            user_id = user_data.get("id") if user_data else response.get("id")
            user_email = user_data.get("email") if user_data else response.get("email")
            
            if not user_id:
                self._send_error_response(500, f"User creation failed. Supabase response: {response}")
                return
            ...
            self._send_json_response(200, {
                ...
                "email_confirmation": {
                    "required": response.get("confirmation_sent_at") is not None,
                    "confirmation_sent_at": response.get("confirmation_sent_at"),
                    "email_verified": user_data.get("email_verified", False)
                },
                "note": "User created in Supabase auth.users table. Email verification status included."
            })
            ...
            if "User already registered" in error_message:
                ...
                self._send_error_response(400, f"Email validation failed: {email}. Try a real email address.")
            else:
